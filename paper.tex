...
For the PageRank algorithm, only the source url and the dest url is needed.
Therefore the dataset was was preprocessed to filter out only the necessary
information.  If the entire dataset is assumed to be represented by a huge
graph, then here each DBPedia instance maps to a Wikipedia article or a page
which inturn could be mapped to a vertex of the graph. The links between the
instances could then be mapped to edges of the graph.
