...
Finally, we will focus on potential data skew problem, which exists in systems
like Hadoop. Since with data skew, a straggling worker could slow down the whole
program considerably.\\

We ran the Bagel version of PageRank on four nodes with a dataset of 30 million
links. As shown in Figure \ref{fig:dataskew}, the memory used is quite balanced
across the nodes. We used the Spark monitoring logs to confirm that the number
of bytes read from disk by each node were exactly the same. So the slight skew
we observed was created later by the RDDs, which are partitioned by a certain
key. \\
